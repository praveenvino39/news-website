...
def homepage(request):
    ...
    weatherdata = requests.get(
        'http://api.openweathermap.org/data/2.5/weather?q=chennai&appid={}'.format(weather_map_api))
    weatherdata = json.loads(weatherdata.text)
    weatherdata = weatherdata['main']
    response = requests.get('https://newsapi.org/v2/top-headlines?country=in&apiKey={}'.format(news_api))
...
def detail(request, news_id, search_term):
    ...
    return render(request, 'homepage/detail.html',
                  {'selected_news': selected_news, 'meta_title': selected_news['title'], 'id': news_id, 'high': high,
                   'low': low, 'search_term': 'default'})
...
def saved_news(request, news_id, search_term):
    ...
    if selected_news['content'] is None:
        ...
        new_save = SavedNew(user=users, title=selected_news['title'], content=selected_news['content'],
                            description=selected_news['description'], urlToImage=selected_news['urlToImage'],
                            url=selected_news['url'], date=selected_news['publishedAt'],
                            source=selected_news['source']['name'])
        new_save.save()
        messages.success(request, 'News added to saved!', extra_tags='success')
...
def saved_list(request):
    ...
    return render(request, 'homepage/saved_list.html',
                  {'content': contents, 'meta_title': 'Snack Time News - Your Picks'})
...
def saved_detail(request, id):
    ...
    if selected_news.user == request.user.username:
        return render(request, 'homepage/saved_detail.html',
                      {'selected_news': selected_news, 'meta_title': selected_news.title})
    else:
        return redirect('homepage')
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        url = 'https://newsapi.org/v2/everything?q={}&apiKey={}'.format(search_term, news_api)
        ...
        return render(request, 'homepage/search.html',
                      {'content': response, 'high': high, 'low': low, 'search_term': request.POST["search"],
                       'search_term': search_term, 'meta_title': 'Snack Time News - {}'.format(search_term)})
    else:
        return redirect('homepage')
...
def search_detail(request, news_id, search_term):
    ...
    return render(request, 'homepage/detail.html',
                  {'selected_news': selected_news, 'meta_title': selected_news['title'], 'id': news_id, 'high': high,
                   'low': low, 'search_term': search_term})
